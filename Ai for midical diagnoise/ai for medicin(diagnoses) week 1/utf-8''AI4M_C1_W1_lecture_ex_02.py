...
loss_reg_1_p = -1 * np.sum(y_true * np.log(y_pred_1)) 
loss_reg_1_n=               -1 * np.sum((1 - y_true) * np.log(1 - y_pred_1))
loss_reg_1=loss_reg_1_p+loss_reg_1_n
print(f"loss_reg_1: {loss_reg_1:.4f}")
print(f"loss_reg_1_p: {loss_reg_1_p:.4f}")
print(f"loss_reg_1_n: {loss_reg_1_n:.4f}")
...
# set the negative weights as the fraction of positive labels (0) for each class (each column)
w_n= np.sum(y_true == 1,axis=0) / y_true.shape[0]
w_n
...
# Set model predictions where all predictions are the same
y_pred = np.ones(y_true.shape)
y_pred[:,0] = 0.5 * y_pred[:,0]
# y_pred[:,1] = 0.5 * y_pred[:,1]
y_pred
